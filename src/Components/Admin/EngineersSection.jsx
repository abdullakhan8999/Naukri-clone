...
const EngineersSection = () => {
  ...
  //functions
  const handleEngineersInfo = (id) => {
    navigate(`/userInfo/${id}`);
  };

  const handleDeleteEngineers = async (id) => {
  ...
  const handleEngineerStatusChange = (e) => {
    if (e.target.value === "") return;
    setEngineersStatus(e.target.value);
  };
  const updateEngineersStatus = (e) => {
    setShowAlert(true);
    setShowUpdateEngineer(false);
  };

  const handleUpdateEngineers = (user) => {
    setName(user.name);
    setRole(user.role);
